  ...
  return (
    <div className={`block ${lesson.type} ${className ? className : ""}`} style={style}>
      <div className="content-left">
        ...
        {lesson.teachers && lesson.teachers.length > 0 && <div>{lesson.teachers.length <= 1 ? lesson.teachers[0].toUpperCase() : `${lesson.teachers[0].toUpperCase()},${lesson.teachers[1].toUpperCase()}${lesson.teachers.length > 2 ? "+" : ""}`}<span className='change'>{lesson.teacherChanged && "!"}</span></div>}
        ...
        <div>{String(new Date(lesson.start * 1000).getHours())}:{String(new Date(lesson.start * 1000).getMinutes()).padStart(2,'0')}-{String(new Date(lesson.end * 1000).getHours())}:{String(new Date(lesson.end * 1000).getMinutes()).padStart(2,'0')}<span className='change'>{lesson.timeChanged && "!"}</span></div>
      </div>
      <div className="content-right">
        {lesson.startTimeSlotName}
