import { Setter, Accessor, Show, createMemo, createSignal, lazy, Suspense } from "solid-js";
import { useAppState } from "../../context";
import { getCurrentDate, getMonday } from "../../utils/functions";
const AddAppointmentModal = lazy(() => import('../Modals/AddAppointmentModal').then(module => ({ default: module.AddAppointmentModal })));
import './Header.css'
      ...
      <>
        <header class="header">
            ...
            <Show when={(!props.showAnnouncements() && !props.showSettings()) || isDesktop()}>
              ...
              <Show when={isDesktop() && settings.enableCustom === "true"}>
                <button class="add-appointment" aria-label="add appointment" onClick={() => setAddAppointmentOpen(!addAppointmentOpen())}><svg viewBox="0 0 1024 1024"><path d="M960 448H576V64a64 64 0 0 0-128 0v384H64a64 64 0 0 0 0 128h384v384a64 64 0 0 0 128 0V576h384a64 64 0 0 0 0-128z" fill="currentColor" /></svg></button>
              </Show>
            </Show>
          </header>

          <Suspense>
            <Show when={addAppointmentOpen()}><AddAppointmentModal setAddAppointmentOpen={setAddAppointmentOpen}/></Show>
          </Suspense>
      </>
    )
}